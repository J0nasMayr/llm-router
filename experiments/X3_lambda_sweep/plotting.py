...
def main():
    ...
    exp_dirs = config_loader.setup_experiment_dirs("X3_lambda_sweep")

    # Load required data files
    summary_df = pd.read_csv(
        results_dir / f"X3_lambda_sweep_summary_stats_{timestamp}.csv"
    )
    run_means_df = pd.read_csv(
        results_dir / f"X3_lambda_sweep_run_means_{timestamp}.csv"
    )
